...
@implementation YXJDDMIndexPath
...
+ (instancetype)indexPathWithCol:(NSInteger)col row:(NSInteger)row {
    YXJDDMIndexPath *indexPath = [[self alloc] initWithColumn:col row:row];
    return indexPath;
}
...
@implementation XYJDDMBackgroundCellView
...
{
    [self selectIndexPath:[YXJDDMIndexPath indexPathWithCol:0 row:0]];
}
...
- (void)selectIndexPath:(YXJDDMIndexPath *)indexPath
{
    ...
    if (indexPath.item < 0 ) {
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:indexPath.row column:indexPath.column] > 0){
            title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[YXJDDMIndexPath indexPathWithCol:indexPath.column row:indexPath.row item:0]];
            [_delegate menu:self didSelectRowAtIndexPath:[YXJDDMIndexPath indexPathWithCol:indexPath.column row:indexPath.row item:0]];
        }else {
...
- (void)setDataSource:(id<YXJDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        if (!self.isClickHaveItemValid && _dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:0 column:i]>0) {
            titleString = [_dataSource menu:self titleForItemsInRowAtIndexPath:[YXJDDMIndexPath indexPathWithCol:i row:0 item:0]];
        }else {
            titleString =[_dataSource menu:self titleForRowAtIndexPath:[YXJDDMIndexPath indexPathWithCol:i row:0]];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        XYJDDMBackgroundCellView *bg = [[XYJDDMBackgroundCellView alloc]init];
    ...
    if (tableView == _leftTableView) {
        if (_dataSourceFlags.titleForRowAtIndexPath) {
            cell.textLabel.text = [_dataSource menu:self titleForRowAtIndexPath:[YXJDDMIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
        } else {
...
//        if ([cell.textLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]])
        ...
        if (_dataSourceFlags.titleForItemsInRowAtIndexPath) {
            NSInteger currentSelectedMenudRow = [_currentSelectRowArray[_currentSelectedMenudIndex] integerValue];
            cell.textLabel.text = [_dataSource menu:self titleForItemsInRowAtIndexPath:[YXJDDMIndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:indexPath.row]];
        } else {
            NSAssert(0 == 1, @"dataSource method needs to be implemented");
        }
        if ([cell.textLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_leftTableView == tableView) {
        ...
        if (isClickHaveItemValid && _delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [self.delegate menu:self didSelectRowAtIndexPath:[YXJDDMIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
        } else {
        ...
        if (self.delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            NSInteger currentSelectedMenudRow = [_currentSelectRowArray[_currentSelectedMenudIndex] integerValue];
            [self.delegate menu:self didSelectRowAtIndexPath:[YXJDDMIndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:indexPath.row]];
        } else {
            //TODO: delegate is nil
...
- (BOOL )confiMenuWithSelectRow:(NSInteger)row {
    ...
    if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:row column:_currentSelectedMenudIndex]> 0) {
        ...
        if (self.isClickHaveItemValid) {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[YXJDDMIndexPath indexPathWithCol:_currentSelectedMenudIndex row:row]];
...
- (void)confiMenuWithSelectItem:(NSInteger)item {
    ...
    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    NSInteger currentSelectedMenudRow = [_currentSelectRowArray[_currentSelectedMenudIndex] integerValue];
    title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[YXJDDMIndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:item]];
    [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
        _show = NO;
    }];
