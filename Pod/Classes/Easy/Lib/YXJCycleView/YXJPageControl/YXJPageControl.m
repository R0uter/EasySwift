...
- (void)initialization
{
    self.dotViewClass           = [YXJAnimatedDotView class];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(YXJPageControl:didSelectPageAtIndex:)]) {
            [self.delegate YXJPageControl:self didSelectPageAtIndex:index];
        }
    }
}
...
- (UIView *)generateDotView
{
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[YXJAnimatedDotView class]] && self.dotColor) {
            ((YXJAnimatedDotView *)dotView).dotColor = self.dotColor;
        }
    } else {
...
{
    if (self.dotViewClass) {
        YXJAbstractDotView *abstractDotView = (YXJAbstractDotView *)[self.dots objectAtIndex:index];
        if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
            [abstractDotView changeActivityState:active];
        } else {
            NSLog(@"Custom view : %@ must implement an 'changeActivityState' method or you can subclass %@ to help you.", self.dotViewClass, [YXJAbstractDotView class]);
        }
